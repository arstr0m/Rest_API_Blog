...
class Post(Base):
    ...
    post_images: Mapped[List["PostImage"]] = relationship("PostImage", back_populates="post")
...
class PostTag(Base):
    ...
    tag_id: Mapped[int] = mapped_column(ForeignKey("tag.id_tag"), primary_key=True)
...
class PostImage(Base):
    ...
    added_at = Column(TIMESTAMP(timezone=True), server_default=text('now()'))
    post_id: Mapped[int] = mapped_column(ForeignKey("posts.id_post"), primary_key=True)
    image_id: Mapped[int] = mapped_column(ForeignKey("images.id_image"), primary_key=True)
